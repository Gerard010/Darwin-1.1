...
def execute_command(command):
    ...
    if re.match(r'https?://(?:[-\w.]|(?:%[\da-fA-F]{2}))+', command):
        ...
        query = command[len("pon "):].strip()  # Obtener lo que sigue a "play"
        if query:
        ...
        query = command[len("busca "):].strip()  # Obtener lo que sigue a "search"
        if query:
    ...
    # Si no se encuentra una respuesta directa
    if response is None:
...
def listen_for_voice_command():
    ...
    with sr.Microphone() as source:
        ...
        recognizer.adjust_for_ambient_noise(source, duration=1)  # Ajustar al ruido ambiental
    ...
    try:
        ...
        # Mostrar lo que se ha entendido
        entry.delete(0, tk.END)
        ...
        # Actualizar la etiqueta con la respuesta
...
# Crear un Frame para el Entry y el botón de voz
...
# Crear un botón para escuchar comandos de voz
...
# Botón de ejecución de comandos escritos
